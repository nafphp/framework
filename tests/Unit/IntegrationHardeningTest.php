...
use Naf\Core\{Container, EventManager, ErrorHandler, ResponseEmitter};
...
final class IntegrationHardeningTest extends NafTestCase
{
    public function testNullFactoriesStayRegisteredUntilReset(): void
    {
        foreach ([new Container(), new AutoResolvingContainer(new Container())] as $container) {
            $calls = 0;
            $container->set('optional', static function() use (&$calls) { ++$calls; return null; });
    ...
    {
        $container = new Container(); $cause = new \RuntimeException('configuration missing');
        $container->set('required', static fn() => throw $cause);
        for ($i=0;$i<2;++$i) {
            try { $container->get('required'); $this->fail('Expected failure'); }
            catch (ContainerException $error) { $this->assertSame($cause, $error->getPrevious()); }
            $this->assertTrue($container->has('required'));
        }
    }
    public function testObjectListenerUsesItsInjectedState(): void
    {
        $listener = new class { public array $seen=[]; public function handle(string $value): int { $this->seen[]=$value; return count($this->seen); } };
        $events = new EventManager(); $events->listen('changed', [$listener, 'handle']);
    ...
    public function testOnlyDevEnablesDetailedErrors(): void
    {
        ...
        try {
            ...
            foreach (['dev'=>true,'prod'=>false,'test'=>false,'production'=>false,'staging'=>false,''=>false] as $env=>$expected) {
                $_ENV['APP_ENV']=$env;
                $this->assertSame($expected,$method->invoke(null),$env);
            }
        } finally { if ($previous===null) unset($_ENV['APP_ENV']); else $_ENV['APP_ENV']=$previous; }
    }
    ...
    public function testEmitterReadsBoundedChunksWithoutStringConversion(): void
    {
        ...
        $stream->expects($this->exactly(2))->method('read')->with(8192)->willReturnOnConsecutiveCalls('first','second');
        ob_start();
        (new \ReflectionMethod(ResponseEmitter::class,'writeBody'))->invoke(null,new Response(200,[],$stream));
